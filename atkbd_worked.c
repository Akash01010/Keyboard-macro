...
//look at this line
	...
	if (atkbd->translated) {
		// printk(KERN_INFO "I am translated");
...
//look at this prob doing for difference b/w long keypress and multiple keypress
	if (!(atkbd->release && test_bit(code, atkbd->force_release_mask)))
		if (keycode != ATKBD_KEY_NULL){
			// printk(KERN_INFO "Hi i think i am printing characters on screen1");
	...
	default:
		...
		printk(KERN_INFO "jiffies: %d,keycode:%u,code: %u,value: %d", jiffies,keycode,code,value);
		input_event(dev, EV_KEY, keycode, value);
		input_sync(dev);
		if (value && test_bit(code, atkbd->force_release_mask)) {
			printk(KERN_INFO "Hi i think i am printing characters on screen");
			input_event(dev, EV_MSC, MSC_SCAN, code);
